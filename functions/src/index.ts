...
const hfConfig = functions.config().huggingface;
console.log('Configuração Hugging Face:', { hasKey: !!hfConfig?.key, keyLength: hfConfig?.key?.length });

if (!hfConfig?.key) {
  console.error('Chave do Hugging Face não encontrada!');
}

export const askIA = functions.https.onRequest((req, res) => {
  corsHandler(req, res, async () => {
    if (req.method !== 'POST') {
    ...
    if (!question) {
      res.status(400).json({ answer: 'Pergunta não enviada.' });
      return;
    }
    try {
      // Chamar o modelo facebook/bart-large-cnn via Hugging Face
      const response = await fetch('https://api-inference.huggingface.co/models/facebook/bart-large-cnn', {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${hfConfig.key}`,
          'Content-Type': 'application/json',
        },
        body: JSON.stringify({
          inputs: `Responda sempre em português do Brasil. Pergunta: ${question}`
        })
      });
      const data = await response.json();
      if (Array.isArray(data) && data[0]?.generated_text) {
        res.json({ answer: data[0].generated_text.trim() });
      } else if (data?.generated_text) {
        res.json({ answer: data.generated_text.trim() });
      } else {
        res.status(500).json({ answer: 'Resposta da IA não disponível.', raw: data });
      }
      return;
    } catch (e) {
      console.error('Erro detalhado do Hugging Face:', e);
      const errorMessage = e instanceof Error ? e.message : 'Erro desconhecido';
      res.status(500).json({ 
        answer: `Erro ao consultar IA: ${errorMessage}`,
        error: errorMessage 
      });
      return;
    }
